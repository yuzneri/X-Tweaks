 ...
 *
 * The rail comes in three shapes, X marking none of them as such. Measured on saved copies
 * of the three pages (see the design document):
 *
 ...
 *
 * On `/search` the form takes the filters' place rather than standing beside them: what X
 * offers there — from anyone or people you follow, anywhere or near you, a link to its own
 * advanced search — is part of what this form asks for, and leaving both would put the same
 * two questions on screen twice.
 */

const RAIL = '[data-testid="sidebarColumn"]';

/**
 * The mark on the panel this feature puts in the rail. Named here rather than only where
 * the panel is built, because deciding where the panel goes means being able to tell it
 * apart from X's own blocks — see `withoutOurs`.
 */
 ...
 * nothing and its heading is a word that changes with the language X is being read in.
 */
...
/**
 * Which of the three shapes this rail is, given what it holds. Kept apart from the DOM so
 * the rule can be read and tested on its own. The search box decides first: a rail holding
 * one is a timeline's rail, whatever else may be in it.
 */
 ...
 * levels below that — and the wrong branch there is silent. Climbing stops at the first
 * ancestor with a sibling, the block the small print is, so its parent is the container
 * (measured on all three pages).
 */
...
/**
 * Which of the container's own children holds this marker. Asked downwards rather than
 * climbed up from the marker: climbing stops at the first ancestor with a sibling, which
 * inside a block is not the block — X's search filters hold three things side by side, so
 * climbing from the link inside them stops two levels short and returns something the
 * container has never heard of.
 */
 ...
 * height the form is not taking up, the form being out of the flow. Told by being empty,
 * which is all there is to go on; getting it wrong costs the form one place in the order
 * and nothing else.
 */
...
/**
 * Whether an element is the heading X puts above its search filters. Named by where it sits
 * — immediately before the block holding the link — since X marks it with nothing and its
 * words change with the language. Refusing anything that holds a link keeps a block of X's
 * from being hidden by mistake if the arrangement changes: every other block in the rail
 * carries links, and a heading carries none.
 */
const isFilterHeading = (element: Element): boolean => element.querySelector('a') === null;

/**
 * The same element, or the one after it where it is the panel already standing there: where
 * the form goes must not depend on whether the form is already there. Without this, a panel
 * standing in exactly the right place answers "put it before itself" — on a timeline the
 * step past the spacer lands on the panel, and at the head of the rail the first child *is*
 * the panel. The insertion then moves it, and a moved node is taken out of the document and
 * put back, dropping the caret out of whatever field was being typed in.
 */
...
export type Placement = {
  ...
  before: Element | null;
  /**
   * X's own blocks that this form stands in for, hidden rather than removed: every
   * rail-block selector in `appearance/css.ts` refuses anything holding the search form,
   * and taking a node out of the rail could put a whole block within reach of a rule meant
 ...
 * cannot be made sense of. Called on every settling of the DOM, so it does no more than a
 * handful of lookups.
 */
