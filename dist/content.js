...
(() => {
  ...
  var en = {
    ...
    transfer: {
      ...
      errors: {
        ...
        badVersion: (version, supported2) => `These settings are in a different format (version ${version}) from what this extension reads (version ${supported2}), so they cannot be loaded`,
    ...
    unreadable: {
      newer: (stored, supported2) => `The saved settings use format version ${stored}, which is newer than the version this extension supports (${supported2}), so they cannot be read. Your settings are still saved. Please update the extension.`,
      older: (stored, supported2) => `The saved settings use format version ${stored}; this extension uses version ${supported2}, and the two are not compatible, so it started from the defaults. The old settings are still stored and are replaced only once you change something here.`
    },
  ...
  var ja = {
    ...
    transfer: {
      ...
      errors: {
        ...
        badVersion: (version, supported2) => `設定の形式（version ${version}）が、この拡張の扱う形式（version ${supported2}）と違うため読み込めません`,
    ...
    unreadable: {
      newer: (stored, supported2) => `保存されている設定の形式（version ${stored}）が、この拡張の対応する形式（version ${supported2}）より新しいため読み込めません。設定は保存されたまま残っています。拡張を更新してください。`,
      older: (stored, supported2) => `保存されている設定の形式（version ${stored}）は、この拡張の形式（version ${supported2}）と互換性がないため、初期状態で始めました。以前の設定は保存されたまま残っていて、ここで何かを変えたときに置き換わります。`
    },
    unset: "未指定",
  ...
  var ruleName = (rule3, m3) => rule3.label || rule3.conditions.map((c3) => describeCondition(c3, m3)).join(m3.conditions.and);
  ...
  var syncOrder = (order, rules) => {
    const ids = new Set(rules.map((rule3) => rule3.id));
    const kept = [];
    ...
    return [...kept, ...rules.map((rule3) => rule3.id).filter((id) => !seen.has(id))];
  };
  ...
  var fillColumn = (v3) => {
    ...
    const column2 = { columnId, account: str2(v3.account), title: str2(v3.title) };
    return v3.missing === true ? { ...column2, missing: true } : column2;
  };
  var fillDeck = (v3) => {
    if (!isRecord(v3)) return null;
    ...
    return {
      deckId,
      name: str2(v3.name),
      columns: columns.map(fillColumn).filter((column2) => column2 !== null)
    };
  };
  ...
  var keepKnown = (fresh, known) => known ? {
    columnId: fresh.columnId,
    account: fresh.account ?? known.account,
    title: fresh.title ?? known.title
  } : fresh;
  var mergeColumns = (known, fresh, configured, rebuild) => {
    const visible = new Set(fresh.map((column2) => column2.columnId));
    const trailing = /* @__PURE__ */ new Map();
    let anchor = null;
    for (const column2 of known) {
      if (visible.has(column2.columnId)) {
        anchor = column2.columnId;
        continue;
      }
      if (rebuild && !configured.has(column2.columnId)) continue;
      const kept = rebuild && !column2.missing ? { ...column2, missing: true } : column2;
      const behind = trailing.get(anchor);
      if (behind) behind.push(kept);
      else trailing.set(anchor, [kept]);
    }
    const knownById = new Map(known.map((column2) => [column2.columnId, column2]));
    const merged = [];
    const placed = /* @__PURE__ */ new Set();
    const place = (column2) => {
      if (placed.has(column2.columnId)) return;
      placed.add(column2.columnId);
      merged.push(column2);
    };
    for (const column2 of trailing.get(null) ?? []) place(column2);
    for (const column2 of fresh) {
      place(keepKnown(column2, knownById.get(column2.columnId)));
      for (const behind of trailing.get(column2.columnId) ?? []) place(behind);
    }
    return merged;
  };
  var mergeDetected = (previous, decks, currentDeckId2, columns, configured, rebuild) => {
    const before = new Map(previous.decks.map((deck) => [deck.deckId, deck]));
    const listed = decks.length > 0 ? decks : previous.decks.map(({ deckId, name }) => ({ deckId, name }));
    const known = (deckId) => before.get(deckId)?.columns ?? [];
    const merged = listed.map(({ deckId, name }) => ({
      deckId,
      name,
      /*
       ...
       */
      columns: deckId !== currentDeckId2 || columns.length === 0 ? known(deckId) : mergeColumns(known(deckId), columns, configured, rebuild)
    }));
    if (currentDeckId2 && !merged.some((deck) => deck.deckId === currentDeckId2)) {
      merged.push({ deckId: currentDeckId2, name: null, columns });
    }
    return { decks: merged, currentDeckId: currentDeckId2 };
  };
  var withoutColumn = (detected, columnId) => ({
    ...detected,
    decks: detected.decks.map((deck) => ({
      ...deck,
      columns: deck.columns.filter((column2) => column2.columnId !== columnId)
    }))
  });
  var allColumns = (detected) => detected.decks.flatMap((deck) => deck.columns);
  var recordable = (columns) => columns.flatMap(
    (column2) => column2.columnId === null ? [] : [{ columnId: column2.columnId, account: column2.account, title: column2.title }]
  );

  // src/filter/health.ts
  var MARKERS = ["cell", "post", "text", "column"];
  var emptyTally = () => ({ articles: 0, cells: 0, posts: 0, texts: 0 });
  var SAMPLE = 20;
  var brokenMarkers = (tally2) => {
    const broken2 = [];
    if (tally2.articles >= SAMPLE && tally2.cells === 0) broken2.push("cell");
    if (tally2.cells >= SAMPLE && tally2.posts === 0) broken2.push("post");
    if (tally2.posts >= SAMPLE && tally2.texts === 0) broken2.push("text");
    return broken2;
  };
  var columnsUnresolved = (total, resolved2) => total > 0 && resolved2 === 0;
  var textRulesStopped = (broken2) => broken2.includes("text");
  var sameMarkers = (a3, b2) => a3.length === b2.length && a3.every((marker, index) => marker === b2[index]);
  ...
  var saveHealth = async (broken2) => {
    await api.storage.local.set({ [HEALTH_KEY]: broken2 });
  };
  ...
  var saveDetected = async (decks, currentDeckId2, columns, configured, rebuild) => {
    const stored = (await api.storage.local.get(DETECTED_KEY))[DETECTED_KEY];
    const previous = fillDetected(stored);
    const next = mergeDetected(previous, decks, currentDeckId2, columns, configured, rebuild);
  ...
  var tiersFor = (settings, scope) => {
    ...
    const column2 = scope.columnId !== null ? settings.columns[scope.columnId] : void 0;
    if (column2) tiers.push(column2);
    return tiers;
  };
  ...
  var appearanceFor = (settings, scope) => {
  ...
  var sourceOf = (settings, scope, pick) => {
    const column2 = scope.columnId !== null ? settings.columns[scope.columnId] : void 0;
    if (column2 && pick(column2) !== null) return "column";
    const account = scope.account !== null ? settings.accounts[scope.account] : void 0;
  ...
  // src/columns/deck.ts
  var NOT_A_DECK = /* @__PURE__ */ new Set(["manage", "new"]);
  var deckIdOf = (url) => {
  ...
  var deckNameOf = (label) => {
    const text = label ?? "";
    for (const [open2, close2] of QUOTES) {
      const start2 = text.indexOf(open2);
      const end = text.lastIndexOf(close2);
      if (start2 < 0 || end <= start2 + 1) continue;
      const name = text.slice(start2 + 1, end).trim();
      if (name) return name.slice(0, 40);
    }
    return null;
  };
  var UNKNOWN_DECK = "?";
  var deckStateFrom = (items, current2) => {
    const linked = items.map((item) => deckIdOf(item.href ?? ""));
    if (items.length === 0) return { decks: [], deckId: current2 };
    if (linked.includes(current2)) return { decks: [], deckId: null };
    const found = [];
    items.forEach((item, index) => {
      const id = linked[index];
      if (id) {
        found.push({ deckId: id, name: deckNameOf(item.label) });
      } else if (item.tagName === "BUTTON" && !found.some((deck) => deck.deckId === current2)) {
        found.push({ deckId: current2, name: deckNameOf(item.label) });
      }
    });
    if (!found.some((deck) => deck.deckId === current2)) {
      found.push({ deckId: current2, name: null });
    }
    return { decks: found, deckId: current2 };
  };
  ...
  var TITLE_SELECTOR = '[data-testid="column-title-wrapper"]';
  var AVATAR_PREFIX = "UserAvatar-Container-";
  var COLUMN_ID_ATTR = "data-xpro-column-id";
  var EMPTY = { account: null, columnId: null };
  var resolved = /* @__PURE__ */ new WeakMap();
  var sequence = 0;
  var askMainWorld = (timeoutMs = 3e3) => new Promise((resolve2, reject) => {
  ...
  var columnScopeOf = (columnEl, maxDepth = 10) => {
    let scope = columnEl;
  ...
  var titleOf = (columnEl) => {
    const title = columnScopeOf(columnEl).querySelector(TITLE_SELECTOR);
    const text = title?.textContent?.trim();
    return text ? text.slice(0, 40) : null;
  };
  var columnElements = () => Array.from(document.querySelectorAll(COLUMN_SELECTOR));
  var DECK_LINK = 'a[href*="/i/decks/"]';
  var DECK_NEW = /\/i\/decks\/new$/;
  var currentDeckId = () => deckIdOf(location.pathname) ?? UNKNOWN_DECK;
  var railOf = () => {
    const links = Array.from(document.querySelectorAll(DECK_LINK));
    const anchor = links.find((link) => deckIdOf(link.getAttribute("href") ?? "") !== null) ?? links.find((link) => DECK_NEW.test(link.getAttribute("href") ?? ""));
    return anchor?.parentElement ?? null;
  };
  var deckState = () => deckStateFrom(
  ...
  var columnSignature = () => columnElements().map((el) => el.getAttribute(COLUMN_ID_ATTR) ?? "?").join("\n");
  var stampIds = async (attempts = 3, waitMs = 400) => {
    for (let attempt = 0; attempt < attempts; attempt++) {
      try {
        const response = await askMainWorld();
        if (!response.error && response.columns.some((column2) => column2.columnId !== null)) return;
      } catch {
      }
      if (attempt < attempts - 1) await new Promise((resolve2) => setTimeout(resolve2, waitMs));
    }
  };
  var readScope = (column2) => ({
    columnId: column2.getAttribute(COLUMN_ID_ATTR),
    account: accountOf(column2)
  });
  var scopeOfElement = (column2) => {
    const known = resolved.get(column2);
    if (known) return known;
    const scope = readScope(column2);
    resolved.set(column2, scope);
    return scope;
  };
  var detect = () => columnElements().map((element) => ({ ...scopeOfElement(element), title: titleOf(element) }));
  var refresh = async () => {
  ...
  var scopeOf = (cell) => {
    const column2 = cell.closest(COLUMN_SELECTOR);
    if (!column2) return EMPTY;
    return scopeOfElement(column2);
  };
  var scopeSettled = (cell) => {
    const column2 = cell.closest(COLUMN_SELECTOR);
    if (column2 === null) return true;
    const columnId = column2.getAttribute(COLUMN_ID_ATTR);
    if (columnId === null) return false;
    const known = resolved.get(column2);
    return known === void 0 || known.columnId === columnId;
  };
  var scopeOfColumn = (column2) => scopeOfElement(column2);
  var scopes = () => columnElements().map(scopeOfElement);
  var drawerColumnIds = async () => {
    try {
      const response = await askMainWorld();
  ...
  var scopeElementOf = (column2) => columnScopeOf(column2);

  // src/filter/decide.ts
  var caseSensitivityOf = (condition2) => isScreenNameTarget(condition2.target) ? false : condition2.caseSensitive;
  var textMatcher = (condition2) => {
    const caseSensitive = caseSensitivityOf(condition2);
    if (isSelfMode(condition2.mode)) {
      return (post) => {
        const me = post.values.screenName[0];
        return post.values[condition2.target].some((value) => sameUser(value, me));
      };
    }
    if (condition2.mode === "regex") {
      let regex;
      try {
        regex = new RegExp(condition2.pattern, caseSensitive ? "u" : "iu");
      } catch {
        return () => false;
      }
      return (post) => post.values[condition2.target].some((value) => regex.test(value));
    }
    const needle = caseSensitive ? condition2.pattern : condition2.pattern.toLowerCase();
    return (post) => post.values[condition2.target].some((raw) => {
      const value = caseSensitive ? raw : raw.toLowerCase();
      return condition2.mode === "exact" ? value === needle : value.includes(needle);
    });
  };
  var escapeRegex = (pattern) => pattern.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  var rangeFinder = (rule3, caseSensitive) => {
    if (rule3.mode === "exact") {
      const needle = caseSensitive ? rule3.pattern : rule3.pattern.toLowerCase();
      return (value) => {
        const target = caseSensitive ? value : value.toLowerCase();
        return target === needle && value.length > 0 ? [{ start: 0, end: value.length }] : [];
      };
    }
    const source = rule3.mode === "regex" ? rule3.pattern : escapeRegex(rule3.pattern);
    let regex;
    try {
    ...
    return (value) => [...value.matchAll(regex)].filter((match) => match[0].length > 0).map((match) => ({ start: match.index, end: match.index + match[0].length }));
  };
  var sameUser = (a3, b2) => b2 !== void 0 && a3.toLowerCase() === b2.toLowerCase();
  var TRAIT_MATCHERS = {
  ...
  var ageMatcher = (condition2) => {
    const limit = condition2.minutes * 6e4;
    const older = condition2.direction === "older";
    return (post) => {
  ...
  var conditionMatcher = (condition2) => {
    const test = condition2.kind === "trait" ? TRAIT_MATCHERS[condition2.trait] : condition2.kind === "age" ? ageMatcher(condition2) : textMatcher(condition2);
    return condition2.negate ? (post) => !test(post) : test;
  };
  var ruleMatcher = (rule3) => {
    const tests = rule3.conditions.map(conditionMatcher);
    return (post) => tests.every((test) => test(post));
  };
  var emphasesOf = (rule3, color) => rule3.conditions.flatMap(
    (c3) => canEmphasizeWith(c3) ? [{ target: c3.target, ranges: rangeFinder(c3, caseSensitivityOf(c3)), color }] : []
  );
  var isPresent2 = (v3) => v3 !== null && v3 !== void 0;
  var AD_LIMIT = 0.5;
  var AD_SAMPLE = 20;
  var adJudgementBroken = (judged2, ads) => judged2 >= AD_SAMPLE && ads / judged2 > AD_LIMIT;
  var TEXT_TARGETS = /* @__PURE__ */ new Set(["text", "quotedText"]);
  var readsText = (rule3) => rule3.conditions.some((condition2) => condition2.kind === "text" && TEXT_TARGETS.has(condition2.target));
  var compileFilter = (filter, messages2, options = {}) => {
    const byId = /* @__PURE__ */ new Map();
    for (const rule3 of filter.rules) {
      if (!rule3.enabled) continue;
      if (options.withoutText && readsText(rule3)) continue;
      const color = rule3.color ?? defaultColorFor(rule3.action);
      byId.set(rule3.id, {
        matches: ruleMatcher(rule3),
        action: rule3.action,
        color,
        label: ruleName(rule3, messages2),
        emphases: rule3.action === ACTIONS.EMPHASIZE ? emphasesOf(rule3, color) : []
      });
    }
    return {
      ...
      rules: filter.order.map((id) => byId.get(id)).filter(isPresent2)
    };
  };
  var decide = (post, filter) => {
    const verdict = { decision: null, emphases: [] };
    if (!filter.enabled) return verdict;
    for (const rule3 of filter.rules) {
      if (!rule3.matches(post)) continue;
      if (rule3.action === ACTIONS.NOTHING) return { decision: null, emphases: [] };
      if (rule3.action === ACTIONS.EMPHASIZE) {
        verdict.emphases.push(...rule3.emphases);
        continue;
      }
      if (verdict.decision) continue;
      switch (rule3.action) {
        case ACTIONS.COLLAPSE:
          verdict.decision = { action: ACTIONS.COLLAPSE, label: rule3.label };
          break;
        case ACTIONS.HIDE:
          verdict.decision = { action: ACTIONS.HIDE, label: rule3.label };
          break;
        case ACTIONS.HIGHLIGHT:
          verdict.decision = { action: ACTIONS.HIGHLIGHT, color: rule3.color, label: rule3.label };
          break;
      }
    }
    return verdict;
  ...
  var parseColor = (color) => {
    ...
    const at = (i3) => parseInt(hex.slice(i3, i3 + 2), 16);
    return {
      r: at(0),
      g: at(2),
      b: at(4),
  ...
  var parseCssColor = (color) => {
    const match = /^rgba?\(\s*([\d.]+)[\s,]+([\d.]+)[\s,]+([\d.]+)(?:\s*[,/]\s*([\d.]+%?))?\s*\)$/.exec(
      color.trim()
    );
    if (!match) return null;
    const [r3, g2, b2] = [match[1], match[2], match[3]].map(Number);
    const raw = match[4];
    const a3 = raw === void 0 ? 1 : raw.endsWith("%") ? Number(raw.slice(0, -1)) / 100 : Number(raw);
    if ([r3, g2, b2, a3].some((v3) => !Number.isFinite(v3))) return null;
    return { r: Math.round(r3), g: Math.round(g2), b: Math.round(b2), a: Math.min(Math.max(a3, 0), 1) };
  };
  var layer = (base, over) => {
    const mix = (b2, o3) => Math.round(b2 * (1 - over.a) + o3 * over.a);
    return { r: mix(base.r, over.r), g: mix(base.g, over.g), b: mix(base.b, over.b) };
  };
  var luminance = ({ r: r3, g: g2, b: b2 }) => {
    const channel = (value) => {
      const v3 = value / 255;
      return v3 <= 0.03928 ? v3 / 12.92 : ((v3 + 0.055) / 1.055) ** 2.4;
    };
    return 0.2126 * channel(r3) + 0.7152 * channel(g2) + 0.0722 * channel(b2);
  };
  var contrastRatio = (a3, b2) => {
    const [light, dark] = [luminance(a3), luminance(b2)].sort((x3, y3) => y3 - x3);
    return (light + 0.05) / (dark + 0.05);
  };
  var readableTextColor = (background, current2) => {
    const shown = current2 === null ? null : layer(background, current2);
  ...
  var fixIfWorsened = (before, after, current2) => {
    if (readableTextColor(before, current2) !== null) return null;
    return readableTextColor(after, current2);
  };

  // src/appearance/background.ts
  var MAX_ANCESTORS = 40;
  var backgroundBehind = (element, skip) => {
    const stack = [];
    let el = element;
    for (let i3 = 0; el && i3 < MAX_ANCESTORS; el = el.parentElement, i3++) {
      if (el === skip) continue;
      const color = parseCssColor(getComputedStyle(el).backgroundColor);
      if (color === null || color.a === 0) continue;
      if (color.a === 1) {
        let base = { r: color.r, g: color.g, b: color.b };
        for (let j3 = stack.length - 1; j3 >= 0; j3--) base = layer(base, stack[j3]);
        return base;
      }
      stack.push(color);
    }
  ...
  // src/appearance/css.ts
  var COLUMN_ATTR = "data-xpro-column";
  var columnKey = (scope) => {
  ...
  var OPENED_POST_MARK = '[data-testid="tweetTextarea_0"]';
  var MEDIA_TARGETS = [
    '[data-testid="tweetPhoto"]',
    '[data-testid="videoPlayer"]',
    '[data-testid="videoComponent"]'
  ];
  var MEDIA_FRAME_ATTR = "data-xpro-media";
  var TIME_ATTR = "data-xpro-time";
  var TODAY_ATTR = "data-xpro-today";
  var HEADER_ATTR = "data-xpro-header";
  var OPENED_CLASS = "xpro-lines-open";
  var MUTED_COLORS = ["rgb(113, 118, 123)", "rgb(83, 100, 113)", "rgb(139, 152, 165)"];
  var LINK_COLORS = [
    "rgb(29, 155, 240)",
    // blue (default)
    "rgb(255, 212, 0)",
  ...
  var within = (scope, targets) => targets.flat().map((target) => `${scope} ${target}`).join(", ");
  var rule2 = (selector, body) => `${selector} { ${body} }`;
  var cssString = (text) => `'${text.replace(/\\/g, "\\\\").replace(/'/g, "\\'")}'`;
  var columnRules = ({ key, appearance }, parens) => {
    const scope = `[${COLUMN_ATTR}="${key}"]`;
    const { colors, media } = appearance;
    const rules = [];
    if (colors.background) rules.push(rule2(scope, `background-color: ${colors.background} !important;`));
    if (appearance.columnWidth !== null) {
      const width = `${appearance.columnWidth}px`;
      rules.push(rule2(scope, `width: ${width} !important; min-width: ${width} !important; max-width: ${width} !important;`));
    }
    if (appearance.maxLines !== null) {
      ...
      rules.push(
        rule2(
          ...
          `display: -webkit-box !important; -webkit-box-orient: vertical !important; -webkit-line-clamp: ${appearance.maxLines} !important; overflow: hidden !important;`
        )
      );
      rules.push(
        rule2(
      ...
      rules.push(
        rule2(
          ...
          "display: block !important; -webkit-line-clamp: none !important; line-clamp: none !important; max-height: none !important; overflow: visible !important;"
        )
      );
      rules.push(
        rule2(
    ...
    if (appearance.fontSize !== null) {
      rules.push(rule2(within(scope, [TARGETS.text]), `font-size: ${appearance.fontSize}px !important;`));
    }
    if (colors.text) {
      rules.push(rule2(within(scope, [TARGETS.text]), `color: ${colors.text} !important;`));
    }
    if (colors.name) {
      rules.push(rule2(within(scope, [TARGETS.name]), `color: ${colors.name} !important;`));
    }
    if (colors.meta) {
      rules.push(rule2(within(scope, [TARGETS.muted]), `color: ${colors.meta} !important;`));
      MUTED_COLORS.forEach((color) => {
        rules.push(rule2(within(scope, [[`[style*="color: ${color}"] svg`]]), `color: ${color} !important;`));
      });
    }
    if (colors.columnTitle) {
      rules.push(rule2(within(scope, [TARGETS.columnTitle]), `color: ${colors.columnTitle} !important;`));
    }
    if (colors.columnHeader) {
      rules.push(
        rule2(within(scope, [TARGETS.columnHeader]), `background-color: ${colors.columnHeader} !important;`)
      );
      rules.push(rule2(within(scope, [TARGETS.headerInner]), "background-color: transparent !important;"));
    }
    if (colors.background) {
      rules.push(
        rule2(within(scope, [TARGETS.tabBar]), `background-color: ${colors.background} !important;`)
      );
    }
    if (colors.background || colors.columnHeader) {
      rules.push(rule2(within(scope, [TARGETS.avatarBackdrop]), "background-color: transparent !important;"));
    }
    if (colors.link) {
      rules.push(rule2(within(scope, [TARGETS.link]), `color: ${colors.link} !important;`));
    }
    if (colors.border) {
      rules.push(
        rule2(within(scope, [TARGETS.divider]), `border-bottom-color: ${colors.border} !important;`)
      );
    }
    if (media.maxThumbHeight !== null) {
      const maxHeight = `${media.maxThumbHeight}px`;
      rules.push(
        rule2(within(scope, [TARGETS.media]), `max-height: ${maxHeight} !important; overflow: hidden !important;`)
      );
      rules.push(
        rule2(
          within(scope, [TARGETS.mediaFrame]),
          `height: ${maxHeight} !important; padding-bottom: 0 !important; aspect-ratio: auto !important; overflow: hidden !important;`
        )
      );
      rules.push(rule2(within(scope, [TARGETS.insideFrame]), "max-height: 100% !important;"));
      rules.push(rule2(within(scope, [TARGETS.photo]), "object-fit: cover !important;"));
    }
    const timeFormat = timeFormatOf(appearance.timeFormat);
    if (timeFormat !== "relative") {
      const absoluteOnly = (selector) => [
        rule2(`${selector}::after`, `content: attr(${TIME_ATTR}) !important;`),
        rule2(`${selector} > time`, "display: none !important;")
      ];
      if (timeFormat === "absolute") {
        ...
        rules.push(
          rule2(`${today}::before`, `content: attr(${TIME_ATTR}) ${cssString(parens.open)} !important;`),
          rule2(`${today}::after`, `content: ${cssString(parens.close)} !important;`)
        );
    ...
    if (mediaCollapses(media.collapse)) {
      rules.push(
        rule2(within(scope, [TARGETS.media, TARGETS.mediaFrame]), "display: none !important;")
      );
    }
    return rules;
  };
  var buildCss = (columns, parens) => columns.flatMap((column2) => columnRules(column2, parens)).join("\n");

  // src/filter/post.ts
  var CELL_SELECTOR = '[data-testid="cellInnerDiv"]';
  var TWEET = '[data-testid="tweet"]';
  var TWEET_TEXT = '[data-testid="tweetText"]';
  var SOCIAL_CONTEXT = '[data-testid="socialContext"]';
  var AUTHOR_AVATAR = '[data-testid="Tweet-User-Avatar"]';
  var USER_NAME = '[data-testid="User-Name"]';
  var AVATAR_NAME_PREFIX = "UserAvatar-Container-";
  var AVATAR_NAME = `[data-testid^="${AVATAR_NAME_PREFIX}"]`;
  var MEDIA = '[data-testid="tweetPhoto"], [data-testid="videoPlayer"], [data-testid="videoComponent"]';
  var PROFILE_LINK = 'a[role="link"][href^="https://x.com/"]';
  var COMMUNITY_NOTE = '[data-testid="birdwatch-pivot"]';
  var NOTE_RATING_ARROW = '[data-testid="icon-arrow-right"]';
  var CARD = '[data-testid="card.wrapper"]';
  var CAROUSEL = '[data-testid="Carousel-NavRight"]';
  var CARD_DETAIL = '[data-testid$=".detail"]';
  var SPACE_BOX = '[data-testid="wrapperView"]';
  var POLL = '[data-testid="cardPoll"]';
  var POLL_OPEN = `${CARD} [role="radio"]`;
  var POLL_CLOSED = `${POLL} [role="list"]`;
  var SPACE = 'a[href*="/i/spaces/"]';
  var ARTICLE = '[data-testid="article-cover-image"]';
  var AD_BADGE = 'svg path[d^="M19.498 3h-15c-1.381 0-2.5 1.12-2.5 2.5v13"]';
  var authorOf = (root) => {
  ...
  var some = (value) => value === null ? [] : [value];
  var textOf = (root) => Array.from(root.querySelectorAll(TWEET_TEXT), (el) => el.textContent ?? "");
  var postedAtOf = (tweet, quote) => {
  ...
  var quoteFrameOf = (tweet) => {
    const own = tweet.querySelector(AUTHOR_AVATAR);
    return Array.from(tweet.querySelectorAll('[role="link"]')).find((el) => {
  ...
  var repostedByOf = (tweet) => {
    const socialContext = tweet.querySelector(SOCIAL_CONTEXT);
    if (!socialContext) return null;
    const link = Array.from(tweet.querySelectorAll(PROFILE_LINK)).find(
      (el) => el.contains(socialContext)
    );
    return link ? linkedName(link) : null;
  };
  var replyLinksOf = (tweet) => {
    const text = tweet.querySelector(TWEET_TEXT);
    if (!text) return [];
    const socialContext = tweet.querySelector(SOCIAL_CONTEXT);
    return Array.from(tweet.querySelectorAll(PROFILE_LINK)).filter((link) => {
  ...
  var textTargetsIn = (root) => Array.from(root.querySelectorAll(TWEET_TEXT), (element) => ({
    element,
    value: element.textContent ?? "",
    offset: 0
  }));
  var nameTargetIn = (root, wants) => {
    const nameEl = root.querySelector(USER_NAME);
    if (!nameEl) return [];
    for (const span of nameEl.querySelectorAll("span")) {
  ...
  var replyTargetsIn = (tweet) => replyLinksOf(tweet).flatMap((link) => {
    const text = link.textContent ?? "";
    const at = text.indexOf("@");
    return at < 0 ? [] : [{ element: link, value: text.slice(at + 1), offset: at + 1 }];
  });
  var markTargets = (cell, target) => {
  ...
  var replyTo = (links, followsThread, author) => {
    const named = links.map(linkedName).filter((name) => name !== null);
  ...
  // src/filter/emphasis.ts
  var STYLE_ID = "xpro-tweaks-mark-style";
  var NAME_PREFIX = "xpro-mark-";
  var byStyle = /* @__PURE__ */ new Map();
  var styleKey = (color, fg) => `${color}|${fg ?? ""}`;
  var nextName = 1;
  var marked = /* @__PURE__ */ new WeakMap();
  var supported = () => typeof CSS !== "undefined" && CSS.highlights !== void 0;
  var isColor = (color) => /^#[0-9a-f]{6}([0-9a-f]{2})?$/i.test(color);
  var writeStyles = () => {
  ...
  var textNodesOf = (element) => {
    const walker = document.createTreeWalker(element, NodeFilter.SHOW_TEXT);
    const nodes = [];
    for (let node = walker.nextNode(); node; node = walker.nextNode()) nodes.push(node);
    return nodes;
  };
  var piecesOf = (nodes, start2, end) => {
    const pieces = [];
    let seen = 0;
    for (const node of nodes) {
      const length = node.data.length;
      const from = Math.max(start2, seen);
  ...
  var overlaps = (taken, spot) => taken.some((other) => spot.start < other.end && other.start < spot.end);
  var markTarget = (target, emphasis, taken) => {
  ...
  var mark = (cell, emphases, adjustContrast) => {
    unmark(cell);
    if (emphases.length === 0 || !supported()) return;
    const added = [];
    const taken = /* @__PURE__ */ new Map();
    const contrast = /* @__PURE__ */ new Map();
    const fgFor = (element, color) => {
      const here = contrast.get(element) ?? /* @__PURE__ */ new Map();
  ...
  var readableOver = (element, color) => {
    const behind = backgroundBehind(element);
    const emphasis = parseColor(color);
    if (behind === null || emphasis === null) return null;
    const current2 = parseCssColor(getComputedStyle(element).color);
    if (current2 === null) return null;
    return fixIfWorsened(behind, layer(behind, emphasis), current2);
  };
  var isGone = (range) => {
    try {
  ...
  var markReadable = (cell) => {
    ...
    for (const element of cell.querySelectorAll("*")) {
      ...
      const fg = fixIfWorsened(before, after, current2);
      if (fg === null) continue;
  ...
  // src/filter/apply.ts
  var COLLAPSED = "xpro-collapsed";
  var HIDDEN = "xpro-hidden";
  var HIGHLIGHTED = "xpro-highlighted";
  var PLACEHOLDER = "xpro-placeholder";
  var COLOR_VAR = "--xpro-highlight";
  var expanded = /* @__PURE__ */ new WeakSet();
  var isExpandedByUser = (cell) => expanded.has(cell);
  var overThemeColor = (cell, color) => {
    const column2 = cell.closest(`[${COLUMN_ATTR}]`);
    if (column2 === null) return null;
    const highlight = parseColor(color);
    if (highlight === null) return null;
    const parent = cell.parentElement;
    if (parent === null) return null;
    const base = backgroundBehind(parent, column2);
    if (base === null) return null;
    const { r: r3, g: g2, b: b2 } = layer(base, highlight);
    return `rgb(${r3}, ${g2}, ${b2})`;
  };
  var placeholderOf = (cell) => cell.querySelector(`:scope > .${PLACEHOLDER}`);
  var resetDecision = (cell) => {
    cell.classList.remove(COLLAPSED, HIDDEN, HIGHLIGHTED);
    cell.style.removeProperty(COLOR_VAR);
    clearReadable(cell);
  ...
  var buildPlaceholder = (cell, decision, author, messages2) => {
    const placeholder = document.createElement("div");
    placeholder.className = PLACEHOLDER;
    const summary = document.createElement("span");
    summary.className = "xpro-placeholder-text";
    summary.textContent = author ? `@${author}` : messages2.placeholder.post;
    ...
    show.textContent = messages2.placeholder.show;
  ...
  var apply = (cell, verdict, author, messages2, look) => {
    showDecision(cell, verdict.decision, author, messages2, look);
    mark(cell, verdict.emphases, look.adjustContrast);
  };
  var showDecision = (cell, decision, author, messages2, look) => {
    if (!decision) {
      resetDecision(cell);
      return;
    }
    if (decision.action === ACTIONS.HIGHLIGHT) {
      ...
      const color = look.highlightBase === "theme" ? overThemeColor(cell, decision.color) ?? decision.color : decision.color;
      cell.style.setProperty(COLOR_VAR, color);
    ...
    if (existing) {
      ...
      cell.prepend(buildPlaceholder(cell, decision, author, messages2));
    }
    cell.classList.add(COLLAPSED);
  };

  // src/filter/styles.css
  var styles_default = "/*\n * How the filter looks.\n *\n * X's elements are never rewritten; these rules apply only to the classes the\n * extension adds. Disabling the extension takes the classes and this stylesheet with\n * it, returning the display to how it was before installation.\n *\n * !important is there to reliably beat X's own high-specificity rules.\n * The targets are confined below the extension's classes, so nothing else is affected.\n */\n\n/* A collapsed cell keeps only the placeholder the extension added */\n.xpro-collapsed > *:not(.xpro-placeholder) {\n  display: none !important;\n}\n\n/*\n * A hidden cell is taken out of the display entirely.\n * The nodes stay in place, so dropping the rule brings it back on the next judgement.\n */\n.xpro-hidden {\n  display: none !important;\n}\n\n.xpro-placeholder {\n  display: flex;\n  align-items: center;\n  gap: 8px;\n  padding: 8px 16px;\n  border-bottom: 1px solid rgba(128, 128, 128, 0.25);\n  color: rgb(113, 118, 123);\n  font-size: 13px;\n  line-height: 1.4;\n}\n\n.xpro-placeholder-text {\n  flex: 0 0 auto;\n  font-weight: 600;\n  /* Keeps a long name from pushing the button out */\n  max-width: 40%;\n  overflow: hidden;\n  text-overflow: ellipsis;\n  white-space: nowrap;\n}\n\n.xpro-placeholder-reason {\n  flex: 1 1 auto;\n  min-width: 0;\n  overflow: hidden;\n  text-overflow: ellipsis;\n  white-space: nowrap;\n}\n\n.xpro-placeholder-show {\n  flex: 0 0 auto;\n  padding: 2px 12px;\n  border: 1px solid rgba(128, 128, 128, 0.5);\n  border-radius: 999px;\n  background: transparent;\n  color: inherit;\n  font: inherit;\n  cursor: pointer;\n}\n\n.xpro-placeholder-show:hover {\n  border-color: rgb(29, 155, 240);\n  color: rgb(29, 155, 240);\n}\n\n/*\n * A highlight paints the background of the whole cell.\n * The post proper carries a background of its own that would hide it, so that range\n * alone is put back to transparent.\n *\n * The strength comes from the opacity in the color (settable on the settings screen).\n * Every default color is 15%, and letting the background show through keeps the body\n * readable under both light and dark palettes.\n */\n.xpro-highlighted {\n  background-color: var(--xpro-highlight) !important;\n}\n\n.xpro-highlighted article {\n  background-color: transparent !important;\n}\n\n/*\n * When a highlight color is strong enough to make text unreadable, that text alone is\n * shifted toward a readable color.\n *\n * The marker is set by `readable.ts`, and only on text that falls short.\n * The colors live here so that removing the marker restores the original color.\n */\n.xpro-highlighted [data-xpro-fg='dark'] {\n  color: #000000 !important;\n}\n\n.xpro-highlighted [data-xpro-fg='light'] {\n  color: #ffffff !important;\n}\n\n/*\n * The \"Show more\" shown under body text cut off by the line limit.\n * It follows X's button for the same purpose: text only, in the link color.\n */\n.xpro-more {\n  display: block;\n  margin-top: 2px;\n  padding: 0;\n  border: none;\n  background: none;\n  color: rgb(29, 155, 240);\n  font: inherit;\n  cursor: pointer;\n}\n\n.xpro-more:hover {\n  text-decoration: underline;\n}\n";

  // src/filter/styles.ts
  var STYLE_ID2 = "xpro-tweaks-filter-style";
  var injectStyles = () => {
  ...
  // src/appearance/time.ts
  var pad = (n2) => String(n2).padStart(2, "0");
  var isToday = (postedAt, now) => postedAt.getFullYear() === now.getFullYear() && postedAt.getMonth() === now.getMonth() && postedAt.getDate() === now.getDate();
  var describeTime = (postedAt, now, m3) => {
    const hour = pad(postedAt.getHours());
    const minute = pad(postedAt.getMinutes());
    if (isToday(postedAt, now)) {
      return { text: m3.appearance.timeClock(hour, minute), today: true };
    }
    const year = postedAt.getFullYear() === now.getFullYear() ? null : postedAt.getFullYear();
    return {
      text: m3.appearance.timeAbsolute(year, postedAt.getMonth() + 1, postedAt.getDate(), hour, minute),
      today: false
    };
  };
  var timeTextFrom = (datetime, now, m3) => {
    if (!datetime) return null;
    const parsed = Date.parse(datetime);
    return Number.isNaN(parsed) ? null : describeTime(new Date(parsed), now, m3);
  };

  // src/appearance/frame.ts
  var slack = (height) => Math.max(10, height * 0.05);
  var needsFrame = (boxHeight, limit) => boxHeight > 0 && boxHeight > limit;
  var wrapsBox = (boxHeight, rectHeight) => rectHeight >= boxHeight - 1 && rectHeight <= boxHeight + slack(boxHeight);
  var setsHeight = (aspectRatio, paddingBottom, boxHeight) => aspectRatio !== "auto" || parseFloat(paddingBottom) >= boxHeight * 0.5;
  var limitFor = (appearance) => mediaCollapses(appearance.media.collapse) ? 0 : appearance.media.maxThumbHeight;

  // src/appearance/apply.ts
  var STYLE_ID3 = "xpro-tweaks-appearance-style";
  var TITLE_SELECTOR2 = '[data-testid="column-title-wrapper"]';
  var CONTENT_SELECTOR = '[data-testid="multi-column-layout-column-content"]';
  var BAND_SELECTOR = `div:has(${TITLE_SELECTOR2}):not(:has(${CONTENT_SELECTOR}))`;
  var headerOf = (scope) => {
    const bands = [...scope.querySelectorAll(BAND_SELECTOR)];
    return bands.find((el) => !bands.some((other) => other !== el && other.contains(el))) ?? null;
  };
  var stillHeader = (marked2, scope) => {
    if (!marked2.matches(BAND_SELECTOR)) return false;
    for (let el = marked2.parentElement; el && el !== scope; el = el.parentElement) {
  ...
  var stampColumns = () => {
    const live = [];
    document.querySelectorAll(COLUMN_SELECTOR).forEach((column2) => {
      const value = columnKey(scopeOfColumn(column2));
      const scope = scopeElementOf(column2);
      if (scope.getAttribute(COLUMN_ATTR) !== value) scope.setAttribute(COLUMN_ATTR, value);
      live.push(scope);
      const marked2 = scope.querySelector(`[${HEADER_ATTR}]`);
      if (marked2 && stillHeader(marked2, scope)) return;
  ...
  var MEDIA_IN_COLUMN = MEDIA_TARGETS.map((target) => `[${COLUMN_ATTR}] ${target}`).join(", ");
  var TEXT_SELECTOR = '[data-testid="tweetText"], [data-testid="User-Name"]';
  var TWEET_TEXT_SELECTOR = '[data-testid="tweetText"]';
  var QUOTE_SELECTOR = '[role="link"]';
  var X_SHOW_MORE_SELECTOR = '[data-testid="tweet-text-show-more-link"]';
  var MAX_ANCESTORS2 = 12;
  var frameOf = (media, limit) => {
    const box = media.getBoundingClientRect();
    if (!needsFrame(box.height, limit)) return null;
    let wrapper = null;
    let decider = null;
    let el = media.parentElement;
    for (let i3 = 0; el && i3 < MAX_ANCESTORS2; el = el.parentElement, i3++) {
  ...
  var limitOf = (media, columns) => {
    const key = media.closest(`[${COLUMN_ATTR}]`)?.getAttribute(COLUMN_ATTR);
    const appearance = columns.find((column2) => column2.key === key)?.appearance;
    return appearance ? limitFor(appearance) : null;
  };
  var clearMediaFrames = () => document.querySelectorAll(`[${MEDIA_FRAME_ATTR}]`).forEach((el) => el.removeAttribute(MEDIA_FRAME_ATTR));
  var clearTimes = () => document.querySelectorAll(`[${TIME_ATTR}]`).forEach((el) => {
    el.removeAttribute(TIME_ATTR);
    el.removeAttribute(TODAY_ATTR);
  });
  var restampTimes = (columns, messages2) => {
    clearTimes();
    ...
    document.querySelectorAll(`[${COLUMN_ATTR}] time`).forEach((time) => {
      ...
      const shown = timeTextFrom(time.getAttribute("datetime"), now, messages2);
      if (!shown) return;
  ...
  var restampMediaFrames = (columns) => {
    clearMediaFrames();
  ...
  var addShowMore = (columns, messages2) => {
    document.querySelectorAll(`[${COLUMN_ATTR}] ${TWEET_TEXT_SELECTOR}`).forEach((text) => {
      ...
      const maxLines = columns.find((column2) => column2.key === key)?.appearance.maxLines ?? null;
      const lineHeight = lineHeightOf(text);
      const reachesLimit = maxLines !== null && text.clientHeight >= (maxLines - 0.5) * lineHeight;
      const clipped = reachesLimit && text.clientHeight > 0 && text.scrollHeight > text.clientHeight + 2;
      ...
      button.textContent = messages2.showMore;
  ...
  var stampMediaFrames = () => {
    if (lastMessages && lastColumns.some((column2) => timeFormatOf(column2.appearance.timeFormat) !== "relative")) {
      restampTimes(lastColumns, lastMessages);
    } else {
      clearTimes();
    }
    const needsFrames = lastColumns.some((column2) => limitFor(column2.appearance) !== null);
    if (needsFrames) {
    ...
    if (lastMessages && lastColumns.some((column2) => column2.appearance.maxLines !== null)) {
      listenToXShowMore();
  ...
  var applyAppearance = (settings, scopes2, messages2) => {
    stampColumns();
    const columns = [];
    const seen = /* @__PURE__ */ new Set();
    ...
    const css = buildCss(columns, messages2.appearance.timeParens);
    const style = styleElement();
    if (style.textContent !== css) style.textContent = css;
    lastColumns = columns;
    lastMessages = messages2;
    stampMediaFrames();
  };

  // src/filter/engine.ts
  var judged = /* @__PURE__ */ new WeakSet();
  var SETTLE_MS = 50;
  var current = null;
  var messages = messagesFor("en");
  var effective = /* @__PURE__ */ new Map();
  var adopt = (settings) => {
    current = settings;
  ...
  var knownSignature = "";
  var reported = "";
  var keyOf = (scope) => `${scope.account ?? ""} ${scope.columnId ?? ""}`;
  var effectiveFor = (scope, settings) => {
  ...
  var adWatch = { judged: 0, ads: 0 };
  var adBroken = false;
  var tally = emptyTally();
  var broken = [];
  var columnsBroken = false;
  var resetAdWatch = () => {
    adWatch = { judged: 0, ads: 0 };
    tally = emptyTally();
  };
  var ARTICLE_SELECTOR = 'article[role="article"]';
  var warnSaveFailed = (what) => () => {
    hooks?.logStyled(
  ...
  var checkHealth = () => {
    ...
    broken = found;
    hooks?.logStyled(
      `⚠ These markers look broken: ${found.join(", ")}. Parts of the extension that depend on them are off until pro.x.com changes back`,
      "color:#f59e0b"
    );
  ...
  var watchAd = (isAd) => {
    ...
    adBroken = true;
    hooks?.logStyled(
      `⚠ ${adWatch.ads} of ${adWatch.judged} posts looked like ads, which cannot be right. Ad rules are off until you reload pro.x.com`,
      "color:#f59e0b"
    );
  ...
  var judgeNew = (root, onlySettled = false) => {
  ...
  var refreshColumns = async () => {
    ...
    try {
      ...
      columnsBroken = columnsUnresolved(
        columns.length,
        columns.filter((column2) => column2.columnId !== null).length
      );
  ...
  var start = async (settings, given) => {
    injectStyles();
    adopt(settings);
    hooks = given;
    observe();
    ...
    const withId = columns.filter((c3) => c3.columnId !== null).length;
    const withAccount = columns.filter((c3) => c3.account !== null).length;
    if (total === 0) {
      ...
      given.logStyled(
        `Resolved columnId for ${withId} and account for ${withAccount} of ${total} columns. Columns that could not be resolved run with whatever settings do apply`,
        "color:#f59e0b"
      );
    }
  };
  ...
  var BoolSelect = ({ value, effective: effective2, onChange, label, on, off }) => /* @__PURE__ */ u3(
    "select",
    {
      "aria-label": label,
      value: String(value ?? effective2),
      onChange: (e3) => {
        const next = e3.currentTarget.value === "true";
        onChange(next === effective2 ? null : next);
      },
      children: [
  ...
  var useReorder = (onDrop) => {
    ...
    const step = q2(() => {
      ...
      const { rows, from: current2 } = placeOf(grab);
      if (current2 < 0) {
        stop();
        return;
      }
      setFrom(current2);
      setTo(dropIndex(rows, current2, grab.y));
    ...
    h2(() => {
      ...
      return () => document.removeEventListener("keydown", onKeyDown2, true);
    }, [from, stop]);
    const gripProps = (index) => ({
      ...
      onPointerUp: (event) => {
        ...
        const { rows, from: current2 } = placeOf(grab);
        const target = current2 < 0 ? -1 : dropIndex(rows, current2, grab.y);
        stop();
        if (current2 >= 0 && target !== current2) onDrop(current2, target);
      },
  ...
  var toDraft = (rule3) => {
    const texts = {};
    for (const c3 of rule3.conditions) {
    ...
    const age = rule3.conditions.find((c3) => c3.kind === "age");
    return {
      trait: rule3.conditions.find((c3) => c3.kind === "trait") ?? null,
      texts,
      age: age ? (() => {
  ...
  var validate = (draft, existing, m3) => {
    ...
    if (existing.some((rule3) => conditionsKey(rule3.conditions) === key)) {
  ...
  var detailOf = (rule3, m3) => rule3.label ? rule3.conditions.map((c3) => describeCondition(c3, m3)).join(m3.conditions.and) : "";
  var Rules = ({ rules, order, onChange }) => {
    ...
    const current2 = editing && rules.some((rule3) => rule3.id === editing.id) ? editing : null;
    const editingId = current2?.id ?? null;
    h2(() => {
    ...
    const save2 = () => {
      ...
      const result = validate(
        current2.draft,
        rules.filter((rule3) => rule3.id !== current2.id),
        m3
      );
      if ("error" in result) {
        setError(result.error);
      ...
      setError(null);
      onChange(
        rules.map((rule3) => rule3.id === current2.id ? { id: rule3.id, ...result.rule } : rule3),
        order
      );
      setEditing(null);
    };
    const startEditing = (rule3) => {
      setError(null);
      setEditing({ id: rule3.id, draft: toDraft(rule3) });
    };
    const remove = (id) => {
      onChange(rules.filter((rule3) => rule3.id !== id), order.filter((x3) => x3 !== id));
      setEditing(null);
      setError(null);
    };
    const toggle2 = (id, enabled) => onChange(rules.map((rule3) => rule3.id === id ? { ...rule3, enabled } : rule3), order);
    const move = (from, to) => {
      const next = [...order];
    ...
    const byId = new Map(rules.map((rule3) => [rule3.id, rule3]));
    const listed = order.map((id) => byId.get(id)).filter((rule3) => rule3 !== void 0);
    return /* @__PURE__ */ u3("fieldset", { children: [
      ...
      /* @__PURE__ */ u3("ul", { class: "rules", children: [
        ...
        listed.map(
          (rule3, index) => current2?.id === rule3.id ? /* @__PURE__ */ u3("li", { class: "editing", children: /* @__PURE__ */ u3(
            RuleForm,
            {
              draft: current2.draft,
              onDraftChange: (next) => setEditing({ id: rule3.id, draft: next }),
              onSubmit: save2,
              mode: "edit",
              onClose: () => {
                setEditing(null);
              ...
              error,
              onDelete: () => remove(rule3.id)
            }
          ) }, rule3.id) : /* @__PURE__ */ u3(
            "li",
            {
              class: [
                rule3.enabled ? "" : "disabled",
              ...
              children: [
                ...
                /* @__PURE__ */ u3("label", { class: "inline", children: /* @__PURE__ */ u3(
                  ...
                  {
                    type: "checkbox",
                    checked: rule3.enabled,
                    "aria-label": m3.rules.enabled,
                    onChange: (e3) => toggle2(rule3.id, e3.currentTarget.checked)
                  }
                ) }),
                /* @__PURE__ */ u3("div", { class: "rule-body", children: [
                  /* @__PURE__ */ u3("span", { class: "rule-name", children: ruleName(rule3, m3) }),
                  detailOf(rule3, m3) && /* @__PURE__ */ u3("span", { class: "rule-meta", children: detailOf(rule3, m3) }),
                  /* @__PURE__ */ u3(ActionBadge, { action: rule3.action, color: rule3.color })
                ] }),
                /* @__PURE__ */ u3("span", { class: "rule-actions", children: [
                  /* @__PURE__ */ u3(
                    "button",
                    {
                      type: "button",
                      class: "move",
                      "aria-label": m3.rules.moveUp(ruleName(rule3, m3)),
                      disabled: index === 0,
                  ...
                  /* @__PURE__ */ u3(
                    "button",
                    {
                      type: "button",
                      class: "move",
                      "aria-label": m3.rules.moveDown(ruleName(rule3, m3)),
                  ...
                  /* @__PURE__ */ u3(
                    ...
                    {
                      type: "button",
                      disabled: current2 !== null,
                      onClick: () => startEditing(rule3),
  ...
  var TierEditor = ({ node, onChange, tab, onTabChange, inherited, effective: effective2 }) => {
    const m3 = useMessages();
    const updateFilter = (patch) => onChange({ ...node, filter: { ...node.filter, ...patch } });
    const tabs = TABS.filter((key) => key !== "effective" || effective2);
    const current2 = tabs.includes(tab) ? tab : "filter";
    return /* @__PURE__ */ u3(S, { children: [
      /* @__PURE__ */ u3("nav", { class: "tabs", "aria-label": m3.tabs.label, children: tabs.map((key) => /* @__PURE__ */ u3(
        "button",
        {
          type: "button",
          "aria-current": key === current2 ? "true" : void 0,
          class: key === current2 ? "tab current" : "tab",
          onClick: () => onTabChange(key),
          children: m3.tabs[key]
        },
        key
      )) }),
      current2 === "filter" && /* @__PURE__ */ u3(S, { children: [
      ...
      current2 === "appearance" && /* @__PURE__ */ u3(
        Appearance,
        {
      ...
      groups.map((group, index) => /* @__PURE__ */ u3("div", { class: "scope-group", children: [
        ...
        /* @__PURE__ */ u3("ul", { children: [
          ...
          group.entries.map((entry) => {
            ...
            const current2 = key === activeKey;
            return /* @__PURE__ */ u3("li", { children: /* @__PURE__ */ u3(
              "button",
              {
                type: "button",
                class: current2 ? "scope current" : "scope",
  ...
  var exportFileName = (at) => `x-pro-tweaks-${at.getFullYear()}-${pad2(at.getMonth() + 1)}-${pad2(at.getDate())}.json`;

  // src/settings/import.ts
  var reidentify = (node) => {
    const renamed = new Map(node.filter.rules.map((rule3) => [rule3.id, newRuleId()]));
    return {
      ...node,
      filter: {
        ...node.filter,
        rules: node.filter.rules.map((rule3) => ({ ...rule3, id: renamed.get(rule3.id) })),
  ...
  var appearanceRows = (settings, scope, effective2, m3) => {
    const of = (pick) => sourceOf(settings, scope, pick);
    const colors = m3.appearance.colors;
    return [
      {
        label: m3.appearance.columnWidth,
        value: size2(effective2.columnWidth, m3.size.unit),
        tier: of((n2) => n2.appearance.columnWidth)
      },
      {
        label: m3.appearance.fontSize,
        value: size2(effective2.fontSize, m3.size.unit),
        tier: of((n2) => n2.appearance.fontSize)
      },
      {
        label: m3.appearance.maxLines,
        value: size2(effective2.maxLines, m3.appearance.lines),
        tier: of((n2) => n2.appearance.maxLines)
      },
      {
        label: m3.appearance.media.maxThumbHeight,
        value: size2(effective2.media.maxThumbHeight, m3.size.unit),
        tier: of((n2) => n2.appearance.media.maxThumbHeight)
      },
      {
        label: m3.appearance.media.collapse,
        // Unset has a default side too (shown). Say which one is in effect
        value: mediaCollapses(effective2.media.collapse) ? m3.appearance.media.collapseOn : m3.appearance.media.collapseOff,
  ...
  var Effective = ({ settings, scope }) => {
    ...
    const byId = new Map(merged.filter.rules.map((rule3) => [rule3.id, rule3]));
    const rules = merged.filter.order.map((id) => byId.get(id)).filter((rule3) => rule3 !== void 0);
    return /* @__PURE__ */ u3(S, { children: [
      ...
      /* @__PURE__ */ u3("fieldset", { children: [
        ...
        rules.length === 0 ? /* @__PURE__ */ u3("p", { class: "hint", children: m3.effective.noRules }) : /* @__PURE__ */ u3("ol", { class: "effective-rules", children: rules.map((rule3) => /* @__PURE__ */ u3("li", { class: rule3.enabled ? void 0 : "off", children: [
          /* @__PURE__ */ u3("span", { class: "effective-label", children: ruleName(rule3, m3) }),
          /* @__PURE__ */ u3("span", { class: "action-badge", children: m3.actions[rule3.action] }),
          !rule3.enabled && /* @__PURE__ */ u3("span", { class: "rule-meta", children: m3.effective.disabled }),
          /* @__PURE__ */ u3(From, { tier: ruleSourceOf(settings, scope, rule3.id) })
        ] }, rule3.id)) })
      ] }),
      /* @__PURE__ */ u3("fieldset", { children: [
  ...
  var SettingsApp = ({ onLocale, start: start2, exportable } = {}) => {
    ...
    const [scope, setScope] = d2(start2 ?? { tier: "global" });
    ...
    const marked2 = (node, scope2) => hasContent(node) && filterApplies(enabledAt(settings, scope2));
    const accountEntries = T2(() => {
      const live = new Set(detected.map((column2) => column2.account).filter((a3) => a3 !== null));
      ...
      return keys.sort().map((account) => ({
        ...
        label: `@${account}`,
        detail: live.has(account) ? m3.tiers.columnCount(detected.filter((column2) => column2.account === account).length) : void 0,
        unassigned: detecting && !live.has(account),
        configured: marked2(settings.accounts[account], { account, columnId: null })
      }));
    }, [detected, settings, detecting, m3]);
    const deckGroups = T2(() => {
      const all = detected;
      const identityOf = (column2) => JSON.stringify([column2.title, column2.account]);
      ...
      return found.decks.map((deck, index) => ({
        ...
        entries: deck.columns.map((column2) => {
          const identity = identityOf(column2);
          const order = (seen.get(identity) ?? 0) + 1;
          seen.set(identity, order);
          const account = column2.account ? `@${column2.account}` : null;
          const number = duplicated.has(identity) ? m3.tiers.nth(order) : null;
          const missing = column2.missing ? m3.tiers.columnMissing : null;
          return {
            scope: { tier: "columns", key: column2.columnId },
            label: column2.title ?? m3.tiers.unnamedColumn,
            detail: [missing, account, number].filter(Boolean).join(" / ") || void 0,
            unassigned: false,
            configured: marked2(settings.columns[column2.columnId], {
              account: column2.account,
    ...
    const missingColumns = T2(() => {
      const liveIds = new Set(detected.map((column2) => column2.columnId));
      return Object.keys(settings.columns).filter((id) => !liveIds.has(id)).sort().map((id) => ({
        ...
        configured: marked2(settings.columns[id], { account: null, columnId: id })
      }));
    }, [detected, settings, detecting, m3]);
    const globalEntry = T2(
      () => ({
        ...
        unassigned: false,
        configured: marked2(settings.global, { account: null, columnId: null })
      }),
      [settings, m3]
    );
    const groups = T2(() => {
    ...
    const current2 = groups.flatMap((group) => group.entries).find((entry) => scopeKey(entry.scope) === scopeKey(scope)) ?? globalEntry;
    const reassignTargets = T2(() => {
      if (current2.scope.tier === "accounts") {
        return [...new Set(detected.map((column2) => column2.account).filter((a3) => a3 !== null))].filter((account) => !settings.accounts[account]).sort().map((account) => ({ key: account, label: `@${account}` }));
      }
      if (current2.scope.tier === "columns") {
        return detected.filter((column2) => column2.columnId !== null && !settings.columns[column2.columnId]).map((column2) => ({
          key: column2.columnId,
          label: `${column2.title ?? m3.tiers.unnamedColumn}${column2.account ? ` / @${column2.account}` : ""}`
        }));
      }
      return [];
    }, [current2.scope, detected, settings.accounts, settings.columns, m3]);
    const reassign = (to) => {
      const from = current2.scope;
    ...
    const inherited = T2(() => {
      ...
      const account = detected.find((column2) => column2.columnId === target.key)?.account ?? null;
      return inheritedFor(settings, { tier: "columns", account });
    }, [current2.scope, settings, detected]);
    const shown = current2.scope;
    const columnScope = shown.tier === "columns" ? {
      account: detected.find((column2) => column2.columnId === shown.key)?.account ?? null,
      columnId: shown.key
    } : null;
    const nodeOf = (target) => {
    ...
    return /* @__PURE__ */ u3(MessagesProvider, { value: m3, children: /* @__PURE__ */ u3("div", { class: "settings", children: [
      /* @__PURE__ */ u3("header", { children: [
        ...
        exportable && ready2 && /* @__PURE__ */ u3(
          "button",
          {
            ...
            onClick: () => setTransferring((open2) => !open2),
      ...
      broken2.length > 0 && /* @__PURE__ */ u3("p", { class: "warning", children: [
        broken2.map((marker) => m3.health[marker]).join(" "),
        " ",
        m3.health.hint
      ] }),
        ...
        /* @__PURE__ */ u3("div", { class: "pane detail", children: [
          /* @__PURE__ */ u3(
            ScopeHeader,
            {
              ...
              onForget: current2.scope.tier === "columns" ? forgetCurrent : void 0
            },
            scopeKey(current2.scope)
          ),
          /* @__PURE__ */ u3(
            TierEditor,
            {
              node: nodeOf(current2.scope),
              onChange: changeNode,
              tab,
              onTabChange: setTab,
              inherited,
...
.clr-pill .clr-hue {\r
  ...
  var STYLE_ID4 = "xpro-coloris-style";
  ...
  var ready = false;
  var closeColorPicker = () => {
    if (ready) coloris_default.close();
  };
  var swatches = [
  ...
  var setupColorPicker = (messages2) => {
    const picker = messages2.color.picker;
    if (ready) {
      coloris_default({
        el: [],
        clearLabel: picker.clear,
        closeLabel: picker.close,
        a11y: a11yOf(messages2)
      });
      return;
    }
    ready = true;
  ...
  // src/ui/panel.css
  var panel_default = '/*\n * How the settings screen\'s components look.\n *\n * Used on two surfaces: the options page and the in-page panel.\n * Every rule is confined to descendants of `.xpro-ui`, so nothing here leaks out\n * even when placed on an X Pro page.\n */\n.xpro-ui {\n  /* Receives the shell (`.settings`) as a full-height flex item */\n  display: flex;\n  flex-direction: column;\n  color-scheme: light dark;\n  background: var(--bg);\n  color: var(--fg);\n  font: 14px/1.6 system-ui, -apple-system, "Segoe UI", "Hiragino Sans", "Noto Sans JP", sans-serif;\n  --bg: #ffffff;\n  --fg: #0f1419;\n  --muted: #536471;\n  --border: #cfd9de;\n  --accent: #1d9bf0;\n  --error: #f4212e;\n  --surface: #f7f9f9;\n}\n\n@media (prefers-color-scheme: dark) {\n  .xpro-ui {\n    --bg: #15202b;\n    --fg: #f7f9f9;\n    --muted: #8b98a5;\n    --border: #38444d;\n    --surface: #1e2732;\n  }\n}\n\n.xpro-ui h1 {\n  margin: 0;\n  font-size: 20px;\n}\n\n.xpro-ui header {\n  display: flex;\n  /* Needed to drop the notice onto its own line. In a narrow window the other items wrap too */\n  flex-wrap: wrap;\n  align-items: baseline;\n  gap: 12px;\n  margin-bottom: 16px;\n}\n\n/* The language selector. Placed at the right end so it does not mix with the settings fields */\n.xpro-ui .language {\n  display: flex;\n  align-items: baseline;\n  gap: 6px;\n  margin-left: auto;\n  font-size: 13px;\n}\n\n/*\n * The notice goes on its own line. On the same line, a long sentence would squash the heading, the language selector and the button alike.\n * "Could not save" is a notice that never clears, so it would stay squashed.\n * Truncating with an ellipsis is no help either: cut short, the failure would not come across.\n * The width is set with `width` rather than `flex`: how flex shrinks differs between the real device and here.\n */\n.xpro-ui #status {\n  order: 1;\n  width: 100%;\n  /* It takes up a line even when empty. The notice appears on every save, and an extra\n     line would push the two panes below it down */\n  min-height: 1lh;\n  margin: 0;\n  color: var(--accent);\n  font-size: 13px;\n}\n\n/* The could-not-save notice never clears, so it does not share a color with success */\n.xpro-ui #status.failed {\n  color: var(--error);\n  font-weight: 600;\n}\n\n.xpro-ui .warning {\n  padding: 10px 12px;\n  border: 1px solid var(--error);\n  border-radius: 6px;\n  color: var(--error);\n}\n\n/*\n * The settings screen\'s shell. The heading stays put and only the two panes below it scroll.\n * The shell is emitted by `SettingsApp` itself. The nesting differs by placement, so passing\n * the height down is not left to the shape of the outside.\n */\n.xpro-ui .settings {\n  flex: 1;\n  min-height: 0;\n  display: flex;\n  flex-direction: column;\n  overflow: hidden;\n}\n\n.xpro-ui .settings > header,\n.xpro-ui .settings > .warning {\n  flex: 0 0 auto;\n}\n\n/*\n * The two panes. Where settings apply on the left, what to set on the right.\n * The left width is set so that "(column not found right now)" fits within two lines.\n */\n.xpro-ui .panes {\n  flex: 1;\n  min-height: 0;\n  display: grid;\n  grid-template-columns: 200px minmax(0, 1fr);\n  gap: 20px;\n}\n\n/*\n * Each pane scrolls on its own. Past ten columns, only the left moves and the right stays put.\n * The room at the bottom belongs here: what scrolls is inside this, so padding on the shell would not reach.\n */\n.xpro-ui .pane {\n  min-height: 0;\n  overflow-y: auto;\n  padding-bottom: 40px;\n}\n\n/*\n * The right pane\'s two tabs. Stacking filter and appearance vertically would not fit on one screen.\n * To keep them distinct from the list on the left, the selection is shown with an underline.\n */\n.xpro-ui .tabs {\n  display: flex;\n  gap: 4px;\n  margin-bottom: 16px;\n  border-bottom: 1px solid var(--border);\n}\n\n.xpro-ui .tab {\n  padding: 8px 16px;\n  border: none;\n  border-bottom: 2px solid transparent;\n  border-radius: 0;\n  background: none;\n  color: var(--muted);\n  cursor: pointer;\n}\n\n.xpro-ui .tab:hover {\n  color: var(--fg);\n}\n\n.xpro-ui .tab.current {\n  border-bottom-color: var(--accent);\n  color: var(--fg);\n  font-weight: 600;\n}\n\n.xpro-ui fieldset {\n  margin: 0 0 20px;\n  padding: 12px 16px 16px;\n  border: 1px solid var(--border);\n  border-radius: 8px;\n}\n\n.xpro-ui legend {\n  padding: 0 6px;\n  font-weight: 600;\n}\n\n.xpro-ui .hint {\n  margin: 4px 0 12px;\n  color: var(--muted);\n  font-size: 13px;\n}\n\n.xpro-ui .row {\n  display: flex;\n  align-items: center;\n  gap: 8px;\n  margin-bottom: 8px;\n}\n\n.xpro-ui .row > span:first-child {\n  flex: 1;\n}\n\n/* A standalone field with no group box. The gap to the next box is the same as between boxes */\n.xpro-ui .row.filter-toggle,\n.xpro-ui .row.appearance-toggle {\n  margin-bottom: 20px;\n}\n\n.xpro-ui .row.add {\n  flex-wrap: wrap;\n  margin-top: 12px;\n}\n\n/*\n * Import and export. It opens below the two panes, with a wide box for reading.\n * It is not left open, so the shell\'s height is borrowed from the two panes only while it is.\n */\n.xpro-ui .transfer {\n  flex: 0 0 auto;\n  margin-top: 16px;\n  padding-top: 12px;\n  border-top: 1px solid var(--border);\n}\n\n.xpro-ui .transfer textarea {\n  box-sizing: border-box;\n  width: 100%;\n  padding: 8px;\n  border: 1px solid var(--border);\n  border-radius: 6px;\n  background: var(--bg);\n  color: var(--fg);\n  /* JSON reads better with the columns lined up */\n  font: 12px/1.5 ui-monospace, SFMono-Regular, Menlo, Consolas, monospace;\n  resize: vertical;\n}\n\n.xpro-ui .transfer .hint,\n.xpro-ui .transfer .error {\n  margin: 6px 0 0;\n}\n\n.xpro-ui .inline {\n  display: flex;\n  align-items: center;\n  gap: 4px;\n  color: var(--muted);\n  white-space: nowrap;\n}\n\n.xpro-ui input[type="text"] {\n  flex: 1;\n  min-width: 140px;\n  padding: 6px 8px;\n  border: 1px solid var(--border);\n  border-radius: 4px;\n  background: var(--bg);\n  color: var(--fg);\n  font: inherit;\n}\n\n.xpro-ui select {\n  padding: 5px 8px;\n  border: 1px solid var(--border);\n  border-radius: 4px;\n  background: var(--bg);\n  color: var(--fg);\n  font: inherit;\n}\n\n.xpro-ui button {\n  padding: 6px 14px;\n  border: 1px solid var(--border);\n  border-radius: 999px;\n  background: var(--surface);\n  color: var(--fg);\n  font: inherit;\n  cursor: pointer;\n}\n\n.xpro-ui button:hover {\n  border-color: var(--accent);\n  color: var(--accent);\n}\n\n/*\n * An action that cannot be undone (deleting a rule). Only the border and the text go red.\n * Filled in, it would become the most prominent thing on screen even where there is no intent to press it.\n */\n.xpro-ui button.danger {\n  border-color: var(--error);\n  color: var(--error);\n}\n\n.xpro-ui button.danger:hover {\n  border-color: var(--error);\n  background: var(--error);\n  color: #ffffff;\n}\n\n/*\n * The delete row. Kept away from the save and cancel row and pushed to the right end.\n * On the same row, the name field\'s growth would crowd the buttons to the right and invite mispresses.\n */\n.xpro-ui .row.add.remove-row {\n  justify-content: flex-end;\n  margin-top: 12px;\n  margin-bottom: 0;\n}\n\n/* The single confirmation before deleting. Boxed in red so the row itself reads as a warning */\n.xpro-ui .row.add.confirm {\n  align-items: center;\n  padding: 8px 10px;\n  border: 1px solid var(--error);\n  border-radius: 6px;\n}\n\n.xpro-ui .row.add.confirm > span:first-child {\n  flex: 1;\n  color: var(--error);\n  font-size: 13px;\n}\n\n.xpro-ui .rules {\n  margin: 0;\n  padding: 0;\n  list-style: none;\n  counter-reset: rule-order;\n}\n\n/*\n * One row of the list, built in two tiers: what is read on top, the controls below.\n * With two panes the row is about 440px wide, and the name plus ↑↓, edit and delete do not fit across.\n * Leaving the wrapping to chance would split the tiers differently per name length and read badly.\n */\n.xpro-ui .rules li {\n  display: grid;\n  grid-template-columns: auto auto minmax(0, 1fr);\n  align-items: start;\n  gap: 6px 8px;\n  padding: 8px 0;\n  border-bottom: 1px solid var(--border);\n  counter-increment: rule-order;\n}\n\n/*\n * The drag handle. A pattern of dots shows that it can be grabbed.\n * Without `touch-action: none`, grabbing it with a finger is taken by the page\'s scrolling.\n */\n.xpro-ui .grip {\n  grid-area: 1 / 1;\n  align-self: center;\n  width: 12px;\n  height: 18px;\n  background-image: radial-gradient(currentColor 1px, transparent 1.2px);\n  background-position: 1px 2px;\n  background-size: 5px 5px;\n  color: var(--muted);\n  cursor: grab;\n  touch-action: none;\n}\n\n.xpro-ui .rules li.dragging .grip {\n  cursor: grabbing;\n}\n\n/* The row being held. Dimmed so it is distinct from the drop line */\n.xpro-ui .rules li.dragging {\n  opacity: 0.5;\n}\n\n/*\n * The drop marker, drawn with an inset shadow rather than a border.\n * As a `border` it would change the row\'s height the moment it is drawn, shifting the rows below and moving the aim.\n */\n.xpro-ui .rules li.drop-before {\n  box-shadow: inset 0 2px 0 var(--accent);\n}\n\n.xpro-ui .rules li.drop-after {\n  box-shadow: inset 0 -2px 0 var(--accent);\n}\n\n/* The on/off switch goes in the top tier, next to the handle */\n.xpro-ui .rules li > .inline {\n  grid-area: 1 / 2;\n}\n\n/*\n * What is read (name, summary of the conditions, action). The action badge is pinned to the top right.\n * Left to wrapping, the badge\'s position would vary per row with the name\'s length and read badly.\n */\n.xpro-ui .rule-body {\n  grid-area: 1 / 3;\n  display: grid;\n  grid-template-columns: minmax(0, 1fr) auto;\n  align-items: start;\n  column-gap: 8px;\n  row-gap: 2px;\n}\n\n/* The name and the summary of the conditions stack vertically in the left column */\n.xpro-ui .rule-body > .rule-name,\n.xpro-ui .rule-body > .rule-meta {\n  grid-column: 1;\n}\n\n/*\n * The summary of the conditions wraps within its column. `.rule-meta` is `nowrap` by default,\n * but what goes here is a sentence of conditions strung together, which would otherwise make the whole right pane scroll sideways.\n */\n.xpro-ui .rule-body > .rule-meta {\n  white-space: normal;\n  overflow-wrap: anywhere;\n}\n\n/* The action badge is the first line of the right column. Its position does not move however many lines the name takes */\n.xpro-ui .rule-body > .action-badge {\n  grid-column: 2;\n  grid-row: 1;\n}\n\n/* The controls line up at the right end of the lower tier, in the same place on every row */\n.xpro-ui .rule-actions {\n  grid-area: 2 / 3;\n  justify-self: end;\n  display: flex;\n  align-items: center;\n  gap: 6px;\n}\n\n/* Judging goes from the top. They are numbered to show that order */\n.xpro-ui .rule-name::before {\n  content: counter(rule-order) ". ";\n  color: var(--muted);\n  font-variant-numeric: tabular-nums;\n}\n\n.xpro-ui .rules li:last-child {\n  border-bottom: none;\n}\n\n/* A rule\'s name. It can be the conditions strung together, so it wraps when long */\n.xpro-ui .rule-name {\n  min-width: 0;\n  word-break: break-all;\n}\n\n.xpro-ui .rule-meta {\n  color: var(--muted);\n  font-size: 12px;\n  white-space: nowrap;\n}\n\n.xpro-ui .empty {\n  color: var(--muted);\n  font-size: 13px;\n}\n\n.xpro-ui .error {\n  margin: 8px 0 0;\n  color: var(--error);\n  font-size: 13px;\n}\n\n/*\n * On an element with display set, the hidden attribute (the UA\'s display:none) is overridden.\n * To hide an element that carries display:flex, such as .color-field, with hidden from JavaScript,\n * the meaning of hidden is restored here.\n */\n.xpro-ui [hidden] {\n  display: none !important;\n}\n\n/* The action and the color */\n.xpro-ui .color-field {\n  display: flex;\n  align-items: center;\n  gap: 6px;\n}\n\n.xpro-ui input[type="color"] {\n  width: 34px;\n  height: 26px;\n  padding: 0;\n  border: 1px solid var(--border);\n  border-radius: 4px;\n  background: none;\n  cursor: pointer;\n}\n\n/*\n * The color-code box. Its selector is made stronger than `input[type="text"]`.\n * `.xpro-ui .colorcode` (0,2,0) alone loses to `.xpro-ui input[type="text"]` (0,2,1), letting\n * `flex: 1` and `min-width` take effect and stretch the box.\n * The width fits both `#rrggbbaa` (nine characters) and the short dimmed text.\n */\n.xpro-ui input[type="text"].colorcode {\n  /* Specified as the width including padding and border (the same treatment as `.size-box`) */\n  box-sizing: border-box;\n  flex: 0 0 auto;\n  width: 116px;\n  min-width: 0;\n  font-family: ui-monospace, SFMono-Regular, Menlo, monospace;\n}\n\n/* Size input: the box, the unit and the error text */\n.xpro-ui .size-field {\n  display: flex;\n  align-items: center;\n  gap: 8px;\n}\n\n/*\n * Fits the number and the unit into one box. Drawing the border on this container rather than\n * on the input itself keeps the unit inside the box while lining its right edge up with the select and color fields.\n */\n.xpro-ui .size-box {\n  box-sizing: border-box;\n  display: flex;\n  align-items: center;\n  gap: 4px;\n  /*\n   * The width is set with `width` rather than `flex-basis`. When measuring the content width of\n   * the parent (`.size-field`), Firefox does not consult the child\'s `flex-basis` and widens the\n   * parent to the input\'s default width (20 characters), pushing the box to the left of the row.\n   * `min-width: 0` stops that default width from taking effect as a minimum.\n   */\n  flex: 0 0 auto;\n  width: 112px;\n  min-width: 0;\n  padding: 6px 8px;\n  border: 1px solid var(--border);\n  border-radius: 4px;\n  background: var(--bg);\n}\n\n.xpro-ui .size-box:focus-within {\n  border-color: var(--accent);\n}\n\n/*\n * The border is on the container, so the input itself carries neither border nor padding.\n * Its specificity equals the `input[type="text"]` rule, so it is placed later to win\n */\n.xpro-ui input.size {\n  flex: 1;\n  min-width: 0;\n  padding: 0;\n  border: none;\n  background: none;\n  text-align: end;\n  font-variant-numeric: tabular-nums;\n}\n\n.xpro-ui .size-box .unit {\n  flex: 0 0 auto;\n  color: var(--muted);\n  font-size: 13px;\n}\n\n.xpro-ui .action-badge {\n  display: inline-flex;\n  align-items: center;\n  gap: 5px;\n  padding: 2px 8px;\n  border: 1px solid var(--border);\n  border-radius: 999px;\n  color: var(--muted);\n  font-size: 12px;\n  white-space: nowrap;\n}\n\n.xpro-ui .swatch {\n  width: 11px;\n  height: 11px;\n  border: 1px solid var(--border);\n  border-radius: 3px;\n}\n\n/* The list of scopes settings apply to (the left pane) */\n.xpro-ui .scopes .hint {\n  margin: 0 0 12px;\n  font-size: 12px;\n}\n\n.xpro-ui .scope-group + .scope-group {\n  margin-top: 14px;\n}\n\n/* A group\'s heading. Smaller and dimmer than the entries, to distinguish it from what can be pressed */\n.xpro-ui .scope-group h2 {\n  display: flex;\n  align-items: baseline;\n  gap: 6px;\n  margin: 0 0 4px;\n  padding: 0 10px;\n  color: var(--muted);\n  font-size: 12px;\n  font-weight: 600;\n}\n\n/*\n * The note beside the heading (the deck currently on screen).\n * A deck name can get long, so what shrinks is the name rather than this.\n */\n.xpro-ui .scope-group h2 .group-note {\n  flex: 0 0 auto;\n  padding: 0 5px;\n  border: 1px solid var(--border);\n  border-radius: 999px;\n  color: var(--muted);\n  font-size: 10px;\n  font-weight: 500;\n}\n\n.xpro-ui .scope-group ul {\n  display: flex;\n  flex-direction: column;\n  gap: 1px;\n  margin: 0;\n  padding: 0;\n  list-style: none;\n}\n\n.xpro-ui .scope-group .empty {\n  padding: 2px 10px 6px;\n  font-size: 12px;\n}\n\n/*\n * One row of the list. Pressing it switches the right pane to that scope\'s settings.\n * The name and the note stack vertically because the left pane is only 200px: side by side,\n * a long account name would push the column name into wrapping one character at a time.\n */\n.xpro-ui .scope {\n  display: grid;\n  grid-template-columns: minmax(0, 1fr) auto;\n  align-items: center;\n  gap: 0 6px;\n  width: 100%;\n  padding: 6px 10px;\n  border: none;\n  border-radius: 6px;\n  background: none;\n  text-align: start;\n  cursor: pointer;\n}\n\n.xpro-ui .scope:hover {\n  background: var(--surface);\n  color: inherit;\n}\n\n/* The scope currently shown on the right. Pressable rows are listed, so the selected one is painted differently */\n.xpro-ui .scope.current {\n  background: var(--surface);\n  color: var(--fg);\n  font-weight: 600;\n}\n\n.xpro-ui .scope-label {\n  flex: 1;\n  min-width: 0;\n  overflow-wrap: anywhere;\n}\n\n.xpro-ui .scope-detail {\n  color: var(--muted);\n  font-size: 11px;\n  font-weight: normal;\n  overflow-wrap: anywhere;\n}\n\n/* Name and mark on top, note below. A row with no note grows no lower tier, so it does not get taller */\n.xpro-ui .scope > .scope-label {\n  grid-area: 1 / 1;\n}\n\n.xpro-ui .scope > .scope-detail {\n  grid-area: 2 / 1;\n}\n\n.xpro-ui .scope > .scope-dot {\n  grid-area: 1 / 2;\n}\n\n/* The mark on a scope with settings. Makes it clear at a glance what was put where */\n.xpro-ui .scope-dot {\n  flex: 0 0 auto;\n  width: 6px;\n  height: 6px;\n  border-radius: 50%;\n  background: var(--accent);\n}\n\n/* What settings are being viewed. Shown at the very top of the right pane */\n.xpro-ui .scope-header {\n  display: flex;\n  align-items: center;\n  gap: 10px;\n  margin-bottom: 12px;\n  padding-bottom: 10px;\n  border-bottom: 1px solid var(--border);\n}\n\n/* A heading element, but sized to match the surrounding text (its place and weight make it read as a heading) */\n.xpro-ui .scope-header .scope-label {\n  margin: 0;\n  font-size: inherit;\n  font-weight: 600;\n}\n\n.xpro-ui .reassign {\n  margin: 0 0 16px;\n  padding: 10px 12px;\n  border: 1px dashed var(--border);\n  border-radius: 6px;\n}\n\n.xpro-ui .reassign .hint {\n  margin: 6px 0 0;\n}\n\n/* The reorder buttons. They sit inside a list row, so they are smaller than the other buttons */\n.xpro-ui .rules .move {\n  padding: 2px 10px;\n}\n\n.xpro-ui .rules button:disabled {\n  opacity: 0.4;\n  cursor: default;\n}\n\n/* A disabled rule. It is not deleted, so it stays, dimmed */\n.xpro-ui .rules li.disabled .rule-name,\n.xpro-ui .rules li.disabled .rule-meta,\n.xpro-ui .rules li.disabled .action-badge {\n  opacity: 0.45;\n}\n\n/*\n * A condition row (the post\'s trait, the text). It has many fields and wraps (`.row.add` carries wrap).\n * So that a wrapped line is recognizable as belonging to that condition, the whole row is painted to read as one block.\n * The gap within a wrap (4px) is tighter than the gap between rows (8px + 8px) for the same reason.\n */\n.xpro-ui .row.add.condition {\n  row-gap: 4px;\n  margin-top: 8px;\n  padding: 6px 10px;\n  border: 1px solid var(--border);\n  /* Thick on the left only. It separates conditions stacked vertically (the same construction as the delete-confirmation row) */\n  border-left-width: 3px;\n  border-radius: 6px;\n  background: var(--surface);\n}\n\n/* The field\'s name (the target, or "What the post is"). Addressed by row so it beats `.row > span:first-child { flex: 1 }` */\n.xpro-ui .row.add.condition > .field-label {\n  flex: 0 0 auto;\n  /* Wide enough to fit "the quoted display name" without wrapping, so the fields\' left edges line up */\n  min-width: 11em;\n  color: var(--muted);\n  font-size: 13px;\n  white-space: nowrap;\n}\n\n/* The button that opens the add form. While closed, this is all that remains below the list */\n.xpro-ui .open-form {\n  margin-top: 12px;\n}\n\n/*\n * The description of the selected action. It takes two lines\' height so it does not shrink when switched to a shorter description.\n */\n.xpro-ui .action-hint {\n  margin: 4px 0 0;\n  min-height: 2.6em;\n}\n\n/* The row being edited. It holds the form, so the list row\'s layout is undone and things stack vertically */\n.xpro-ui .rules li.editing {\n  display: block;\n  padding: 10px 0;\n}\n\n.xpro-ui .rules li.editing .row.add {\n  margin-top: 0;\n}\n\n.xpro-ui .rules li.editing .row.add + .row.add {\n  margin-top: 8px;\n}\n\n/* The color swatch, showing the color in effect */\n.xpro-ui .color-field .swatch {\n  flex: 0 0 auto;\n  width: 16px;\n  height: 16px;\n}\n\n/*\n * At widths where splitting left and right makes the right unreadable, it goes back to stacking and the shell scrolls.\n * The in-page panel is `min(720px, 100%)`, so a small window lands here.\n */\n@media (max-width: 560px) {\n  .xpro-ui .settings {\n    /* Nothing scrolls inside; the shell scrolls instead. The panes stack, so moving only one of them means nothing */\n    min-height: auto;\n    overflow: visible;\n  }\n\n  .xpro-ui .panes {\n    display: block;\n  }\n\n  .xpro-ui .pane {\n    overflow: visible;\n    padding-bottom: 0;\n  }\n\n  .xpro-ui .scopes {\n    margin-bottom: 20px;\n    padding-bottom: 16px;\n    border-bottom: 1px solid var(--border);\n  }\n}\n\n/*\n * What is in effect. This surface is for reading, so the text is tighter than on the editing surface, to fit more at a glance.\n * A row is three columns: item, value, origin. The origin is aligned to the right end.\n */\n.xpro-ui .effective-row {\n  display: grid;\n  grid-template-columns: minmax(0, 1fr) auto auto;\n  align-items: baseline;\n  gap: 8px;\n  padding: 4px 0;\n  border-bottom: 1px solid var(--border);\n}\n\n.xpro-ui .effective-row:last-child {\n  border-bottom: none;\n}\n\n.xpro-ui .effective-value {\n  font-variant-numeric: tabular-nums;\n}\n\n/* An item no tier sets. It means X Pro\'s own, so it is distinguished from a value */\n.xpro-ui .effective-value.unset {\n  color: var(--muted);\n}\n\n/* Which tier a value came from. It is a hint while reading, so it stays more restrained than the value */\n.xpro-ui .from {\n  padding: 1px 8px;\n  border: 1px solid var(--border);\n  border-radius: 999px;\n  color: var(--muted);\n  font-size: 11px;\n  white-space: nowrap;\n}\n\n/*\n * Numbered in judging order.\n * A `li` made a grid shows no `::marker`, so a counter is used, as on the editing surface (`.rules`).\n */\n.xpro-ui .effective-rules {\n  margin: 0;\n  padding: 0;\n  list-style: none;\n  counter-reset: applied-order;\n}\n\n.xpro-ui .effective-rules li {\n  counter-increment: applied-order;\n}\n\n.xpro-ui .effective-rules .effective-label::before {\n  content: counter(applied-order) ". ";\n  color: var(--muted);\n}\n\n.xpro-ui .effective-rules li {\n  display: grid;\n  grid-template-columns: minmax(0, 1fr) auto auto auto;\n  align-items: baseline;\n  gap: 8px;\n  padding: 6px 0;\n  border-bottom: 1px solid var(--border);\n}\n\n.xpro-ui .effective-rules li:last-child {\n  border-bottom: none;\n}\n\n/* A rule stopped rather than deleted. Its presence is shown, dimmed so it reads as not in effect */\n.xpro-ui .effective-rules li.off {\n  opacity: 0.55;\n}\n\n.xpro-ui .effective-label {\n  overflow-wrap: anywhere;\n}\n';

  // src/panel/frame.css
  var frame_default = "/*\n * The frame of the in-page panel.\n *\n * How the contents look lives in ui/panel.css. This file decides only where and how\n * things are placed.\n * Every rule is confined inside elements the extension added, so X Pro's own display is\n * never touched.\n */\n\n.xpro-panel-host {\n  position: fixed;\n  inset: 0;\n  /* In front of X Pro's popups. X only goes up into the 10000s (confirmed in the real DOM) */\n  z-index: 2147483000;\n  display: flex;\n  justify-content: flex-end;\n  background: rgba(0, 0, 0, 0.4);\n}\n\n/*\n * The color picker (Coloris) is placed outside the panel (directly under body) and\n * carries a stacking order of 1000 of its own. The panel sits above X Pro and therefore\n * uses a vastly larger value, so as it stands the picker would end up behind the panel\n * and pressing it would look like nothing happens.\n * It goes one above the panel (specified by id, so it beats Coloris's own `.clr-picker`).\n */\n#clr-picker {\n  z-index: 2147483001;\n}\n\n/*\n * What scrolls is the panes inside, not this frame.\n * Specifying `overflow` here would later override the shell's own settings in\n * `panel.css` (including the switch for narrow widths), so it is left out.\n */\n.xpro-panel {\n  position: relative;\n  /* Fits within 720px including the padding. As content-box it would extend past it by the padding */\n  box-sizing: border-box;\n  width: min(720px, 100%);\n  height: 100%;\n  padding: 16px 20px 0;\n  box-shadow: -8px 0 24px rgba(0, 0, 0, 0.35);\n}\n\n/*\n * Where Preact renders. It sits beside the close button and cannot go directly under\n * the frame, so this one level is interposed.\n * It passes the panel's height straight to the shell (`.settings` in `ui/panel.css`).\n */\n.xpro-panel > div {\n  flex: 1;\n  min-height: 0;\n  display: flex;\n  flex-direction: column;\n}\n\n/*\n * The close button is layered over the contents. Lined up with the settings fields, it\n * would invite mispresses.\n *\n * The selector starts from `.xpro-panel` so it beats the components' own\n * `.xpro-ui button` (padding and left alignment) on specificity. At equal specificity\n * this one wins by being read later, but that breaks the moment the component CSS is\n * reordered.\n */\n.xpro-panel .xpro-panel-close {\n  position: absolute;\n  /* Aligned with the top of the heading row: the same value as the panel's top padding */\n  top: 16px;\n  right: 16px;\n  width: 32px;\n  height: 32px;\n  padding: 0;\n  border: 1px solid var(--border);\n  border-radius: 999px;\n  background: var(--surface);\n  color: var(--fg);\n  cursor: pointer;\n}\n\n/*\n * The ✕ is drawn with lines rather than as a character.\n *\n * The character \"×\" is drawn on the mathematical axis rather than the center of the\n * line, so centering it in its container still leaves it sitting low, and how far it\n * sits off varies with the typeface.\n * Two lines can be positioned relative to the container's center.\n */\n.xpro-panel .xpro-panel-close::before,\n.xpro-panel .xpro-panel-close::after {\n  content: '';\n  position: absolute;\n  top: 50%;\n  left: 50%;\n  width: 14px;\n  height: 1.5px;\n  background: currentColor;\n}\n\n.xpro-panel .xpro-panel-close::before {\n  transform: translate(-50%, -50%) rotate(45deg);\n}\n\n.xpro-panel .xpro-panel-close::after {\n  transform: translate(-50%, -50%) rotate(-45deg);\n}\n\n.xpro-panel .xpro-panel-close:hover {\n  border-color: var(--accent);\n  color: var(--accent);\n}\n\n/* Room on the heading row for the close button, so it does not overlap the language selector */\n.xpro-panel header {\n  padding-right: 48px;\n}\n\n/* At the width where the layout stacks vertically, the panel's contents grow taller, so the frame itself scrolls */\n@media (max-width: 560px) {\n  .xpro-panel {\n    overflow-y: auto;\n  }\n}\n";

  // src/panel/panel.tsx
  var STYLE_ID5 = "xpro-tweaks-panel-style";
  var HOST_CLASS = "xpro-panel-host";
  var host = null;
  var onKeyDown = (event) => {
    if (event.key !== "Escape") return;
...
${frame_default}`;
  ...
  var open = (start2) => {
    ...
    const applyLocale = (locale) => {
      const messages2 = messagesFor(locale);
      panel.lang = locale;
      closeButton.setAttribute("aria-label", messages2.panel.close);
      setupColorPicker(messages2);
    };
    R(/* @__PURE__ */ u3(SettingsApp, { onLocale: applyLocale, start: start2 }), app);
    document.addEventListener("keydown", onKeyDown, true);
  };
  var toggle = (start2) => isOpen() ? close() : open(start2);
  ...
  // src/panel/menu.ts
  var MENU = '[role="menu"]';
  var DECK_MENU_ANCHOR = '[data-tourstep="side-nav-more-options"]';
  var MENU_ITEM = '[role="menuitem"]';
  var MARK = "data-xpro-menu-item";
  var trigger = null;
  var watchTrigger = (onTrigger) => {
    document.addEventListener(
      "pointerdown",
      (event) => {
        const target = event.target;
  ...
  var isDeckMenu = (menu) => {
    ...
    for (let el = trigger, depth = 0; el && depth <= 2; el = el.parentElement, depth++) {
      if (el.querySelector(DECK_MENU_ANCHOR)) return true;
    }
    return false;
  };
  var buildItem = (template, label) => {
  ...
  var insertInto = (messages2) => {
    ...
    for (const menu of document.querySelectorAll(MENU)) {
      ...
      template.parentElement.append(buildItem(template, messages2.title));
      inserted = true;
    }
    return inserted;
  };

  // src/panel/column-options.ts
  var DRAWER = '[data-testid="drawerAnimatedDiv"]';
  var OPTION_ITEM = '[data-testid^="option-icon"]';
  var MARK2 = "data-xpro-column-options-item";
  var column = null;
  var triggered = null;
  var rememberTrigger = (target) => {
    if (!target || target.closest(`[${MARK2}]`)) return;
  ...
  var drawerColumn = /* @__PURE__ */ new WeakMap();
  var asked = /* @__PURE__ */ new WeakSet();
  var triggeredColumnId = () => (column?.isConnected ? scopeOfColumn(column).columnId : null) ?? triggered;
  var columnIdFor = (item) => {
    const drawer = item.closest(DRAWER);
    const known = drawer ? drawerColumn.get(drawer) : void 0;
    return known ?? triggeredColumnId();
  };
  var isOptionRow = (el) => el.tagName === "BUTTON" && el.querySelector(OPTION_ITEM) !== null;
  var sheetOf = (drawer) => {
    let sheet = null;
    let most = 0;
  ...
  var watch3 = (onOpen) => {
    ...
    document.addEventListener(
      ...
      (event) => {
        const target = event.target;
        if (!(target instanceof Element) || !target.closest(`[${MARK2}]`)) return;
        event.preventDefault();
        event.stopPropagation();
        try {
  ...
  var buildItem2 = (template, label) => {
    const item = template.cloneNode(true);
    item.setAttribute(MARK2, "");
  ...
  var PREFIX_STYLE = "color:#1d9bf0;font-weight:bold";
  var log = (...args) => console.log("%c[X Pro Tweaks]", PREFIX_STYLE, ...args);
  var logStyled = (message, style) => console.log(`%c[X Pro Tweaks]%c ${message}`, PREFIX_STYLE, style);
  var effectiveSettings = (settings, paused) => paused ? { ...emptySettings(), language: settings.language } : settings;
  var summarize = (settings) => ({
  ...
  var main = async () => {
    ...
    saveAdGuard(false).catch(warnSaveFailed2("that ad rules are on again"));
    saveHealth([]).catch(warnSaveFailed2("that the markers are fine again"));
    let current2 = settings;
    let paused = startPaused;
    if (unreadable !== null) {
      logStyled(
        `⚠ The saved settings use format version ${unreadable.version}, which this extension cannot read. Running with defaults; nothing was overwritten`,
        "color:#f59e0b"
      );
    }
    api2.runtime.onMessage.addListener((message) => {
      if (message?.type === OPEN_PANEL) open();
    });
    ...
    let started = false;
    let missed = false;
    let rebuiltDeck = null;
    const reapply = () => {
      if (!started) {
        missed = true;
        return false;
      }
      updateSettings(effectiveSettings(current2, paused));
      return true;
    };
    subscribe((next) => {
      current2 = next;
    ...
    subscribePaused((next) => {
      ...
      log(paused ? "Paused; nothing is applied" : "Resumed", summarize(current2));
    });
    await start(effectiveSettings(current2, paused), {
      log,
      logStyled,
      ...
      onColumns: (found) => {
        ...
        const configured = new Set(Object.keys(current2.columns));
        saveDetected(found.decks, found.deckId, columns, configured, rebuild).catch(
          warnSaveFailed2("the columns found on this page")
        );
      },
      onSettle: () => {
        const messages2 = currentMessages();
        insertInto(messages2);
        insertInto2(messages2);
      }
    });
    started = true;
    log("Filter started", summarize(current2));
    if (missed) {
      updateSettings(effectiveSettings(current2, paused));
      log("Applied settings that changed while starting", summarize(current2));
    }
  ...
  main().catch((e3) => {
    ...
    log(e3 instanceof Error ? e3.message : e3);
  });
})();
